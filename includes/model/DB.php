...
// 
require('DB/Record.php'); // import the DB_Record class
...
class DB {
  ...
  // enables __call()s like find_by_name_like('dom%') to match Dom, domhastings or DOMINIK
  
  // variables
  public $c = null; // the connection
  public $table = null; // the table
  public $id = 'id';  // the id column
  
  private $q = '';  // the last query run
  private $fields = array();  // an array containing the field data of the current table
  
  // these variables are overwritten by the connection string
  private $db = null;  // the database
  private $engine = 'mysql';  // the engine
  private $host = 'localhost';  // the server
  private $user = 'root'; // the username
  ...
  // _get
  // accepts a query string, or mysql resource and returns either an array
  ...
  public function delete($id = null, $options = array()) {
    ...
    if (!empty($options['mode'])) {
      if ($options['mode'] !== "AND" || $options['mode'] !== "OR") {
        $mode = "OR";
      } else {
        $mode = $options['mode'];
      }
